...
### Added
- ステータス表示の日本語化 (階, 金貨, 体力, 強さ, 守備, 経験) - オリジナル日本語版準拠
- 空腹状態表示の実装 (空腹, 飢餓, 瀕死) と色分け (黄, 橙, 赤)
- ステータスバーのGridレイアウト化による表示位置固定
- 状態異常のアイコン表示実装 (💫, 🕶️, 🌈, ❄️, 💤, ⛓️など, 右寄せ配置)
- インベントリ表示を固定行数 (24個+足元) に変更し、サブメニュー表示位置を安定化
- 睡眠ガス罠と凍結攻撃の挙動統一（自動進行）とアイコン分岐 (❄️/💤)
- デバッグ機能: `/`キーでの強制レベルアップ
- バグ修正: レベル上限到達時の無限ループ (ハング) 回避
- バグ修正: 状態異常 (held, bearTrap) の参照ミス修正

### Fixed
- 墓石画面・ランキング画面からのゲーム再開/タイトル遷移の不具合修正 (InputManagerへのステート管理統合)

## 2026-01-15

### 🎮 キー設定 (Key Configuration) の実装
- **キーバインド機能**:
  - タイトルメニューおよび設定画面から詳細なキー割り当てが可能に
  - A/B/X/Y/L/R/SELECT/START および移動キーの割り当てを変更可能
  - `InputManager` クラスでキー設定を管理し、`localStorage` に保存
- **UI/UXの強化**:
  - **SFCカラー**: ボタンラベルの背景色をスーパーファミコン準拠（赤・黄・青・緑）に変更し、視認性を向上
  - **デフォルトリセット**: 「デフォルトに戻す」ボタンを追加し、1クリックで初期設定（SFC配置）に復元可能に
  - **戻るボタン**: 設定画面を抜けてもカーソル位置を記憶、初期位置を常に「戻る」ボタンに固定
- **入力システムの安定化**:
  - 入力処理の競合を解消し、タイトル画面から設定画面、ゲームプレイへの遷移における入力バグを修正
  - 決定キー（Enter）に加え、割り当て後の「Aボタン」でも決定操作ができるように改善

## 2026-01-14

### 🏆 ゲームクリア・ランキングシステム整備
- **売却画面 (Selling Screen) 実装**:
  - ゲームクリア時、所持品を自動的に売却してスコアを加算する画面を実装
  - オリジナルRogue準拠の `Item.getValue` メソッドを追加（エンチャント、チャージ数、スタック数などを考慮）
  - **指輪**: 特定の指輪はエンチャント値に応じて高額査定、それ以外はベース価格
  - **食料**: 食料類は買い取らない（オリジナル準拠）
- **スコア計算ロジック**:
  - 勝利した際、スコア（所持金）から10%が減額される仕様を再現（ギルドへの上納金(?)）
- **不具合修正**:
  - **画面遷移バグ**: 「勝利→売却→ランキング」のフローで、ランキング画面が多重表示される問題を修正
  - **スコア重複登録**: 1回のゲームクリアでスコアが複数回登録される問題を修正（厳密なステート管理）

### 🪦 墓石表示の日本語対応強化
- **全角文字の位置ズレ補正**:
  - 等幅フォント(Noto Sans Mono)環境において、全角文字の幅が半角文字の2倍よりもわずかに狭く表示される問題に対応
  - 全角文字数に応じた動的なパディング補正ロジックを実装
  - プレイヤー名、死因、モンスター名に日本語が含まれていても、墓石のアスキーアートが崩れにくいように調整

### 🎨 UI・メッセージ調整
- **開始メッセージの修正**:
  - ゲーム開始時のメッセージを、オリジナル日本語版準拠の「やあ、%s。 運命の洞窟へようこそ...」に変更
  - タイトル画面に表示されていた独自のウェルカムメッセージ「ようこそ、勇敢なる冒険者よ。」を削除
- **UIデザイン改善**:
  - ステータスバー、インベントリパネル、メッセージログエリアを枠線で囲むスタイルに変更
  - 画面のゾーニングを明確化
- **アイテム操作の修正 (オリジナル準拠)**:
  - 階段や罠の上にアイテムを置けないように修正（「そこには、すでに何かが置かれている。」を表示）
  - 足元にアイテムがある場合、インベントリ一覧で階段が重複して表示されないように修正（アイテム優先）
- **モンスター挙動の修正**:
  - L(金持ち妖精)の攻撃で、最低1ダメージ保証により誤ってダメージが発生していた問題を修正
  - 攻撃力0の場合、命中してもHPダメージが発生しない（特殊効果のみ発動する）ように変更
  - Lが床に落ちている金貨を見つけた際、盗んで消えるのではなく、金貨の上に乗って眠る（WAKENSフラグ削除）ように修正
  - 未定義メソッド `addFlag` の呼び出しを `setFlag` に修正
- **モンスタードロップ機能の実装 (オリジナル準拠)**:
  - モンスターを倒した際にアイテムをドロップする `coughUp` 機能を実装
  - L (Leprechaun): 100%の確率で金貨をドロップ（金額は階層×15〜30）
  - その他のモンスター: 各モンスター固有の `dropPercent` 確率でランダムアイテムをドロップ
    - 例: ドラゴン(D) 90%, メデューサ(M) 25%, ケンタウロス(C) 10% など
  - 帰還中（クリア済み階層）ではドロップしない仕様も再現
- **毒・ステータス関連の修正**:
  - 毒ダーツ罠のメッセージ修正: 誤って「飢餓」と表示されていた問題を修正し、ダメージ時も特別なメッセージを表示しないよう修正（オリジナル準拠）
  - R(がらがらへび)の毒攻撃メッセージ修正: 「力が弱くなった気がする。」の表示を削除（オリジナル準拠）
  - 力（Strength）の減少処理修正: 毒で力が下がっても、毎ターンのステータス再計算で元に戻ってしまうバグを修正
  - ステータスバー更新修正: 力が下がった際に、即座に画面上の数値が更新されるように修正
  - 毒ダーツによる死亡処理を実装（以前はエラーになっていた）
- **氷の怪物(I)のフリーズ修正**:
  - 無限凍結バグの修正: 攻撃を受けるたびに凍結ターン数が加算され、永遠に動けなくなる問題を修正（重なり時は延長のみ）
  - 行動不能処理の実装: 凍結中はプレイヤーの入力を受け付けず、自動的にターンを経過させるように変更
  - 凍結解除時メッセージ: "再び動けるようになった。" と表示されるように修正
- **死霊(W)とバンパイア(V)の特殊攻撃修正**:
  - 死霊(W): レベルダウン攻撃をオリジナルRogue仕様に完全準拠（Lv5以下無効、確率20%、経験値減少量、HP減少量を再現）。
  - バンパイア(V): HP吸収攻撃をオリジナルRogue仕様に完全準拠（MaxHP30以下無効、HP減少に加え、確率で力も減少するように修正）。
- **経験値テーブルの修正**:
  - プレイヤーの経験値テーブルをオリジナルRogueと同じ数値に変更（Lv8以降の必要経験値のズレを修正）。
...
### 🔍 探索仕様の修正 (オリジナルRogue準拠)
- **探索コマンド (s)**:
  - 1回のコマンド実行（1ターン消費）につき、探索判定を2回行うように変更
  - 隠し扉と罠の両方に対して適用され、発見効率が2倍に向上

### 🎨 UI・メッセージ調整
- **視認性向上**:
  - CSSを調整し、壁(`.wall`)の文字色と背景色を反転（茶色背景・黒文字）して見やすく変更
- **メッセージ修正**:
  - プレイヤー攻撃時のメッセージ主語を「あなた」で統一するように修正（例：「あなたの攻撃は命中した」）
  - モンスター攻撃時のメッセージを修正し、「〇〇の攻撃は命中した」と正しく表示されるように変更
