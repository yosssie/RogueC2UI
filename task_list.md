...
### 最新の追加機能 (本日実装)
*   **UI & 操作性の向上**:
    *   **ダッシュ改善**: ダッシュ（Shift移動/Bボタン）時、アイテムを拾わずに停止する処理を実装
    *   **足元情報表示**: インベントリ画面の最下部に、現在足元にあるアイテムや階段を常時表示
    *   **サブメニュー拡張**: 足元のアイテムや階段を選択した際、サブメニューから「拾う」「降りる」を実行可能に
    *   **インベントリ制限**: MAXPACK（23個）制限の実装とメッセージ表示
